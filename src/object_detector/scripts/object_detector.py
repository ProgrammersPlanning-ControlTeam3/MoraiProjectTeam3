...
class object_detector:
    ...
    def nearest_cost(self):
        ...
        if self.object_msg.npc_list:  # npc_list가 비어 있지 않은지 확인
            npc_positions = np.array([(obj.position.x, obj.position.y) for obj in self.object_msg.npc_list])
        else:
            return float('inf'), 0  # NPC가 없는 경우, 무한대 거리와 0의 헤딩 차이를 반환
